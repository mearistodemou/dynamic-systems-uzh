...
initialAffect1 <- rnorm(n = NSubjects, mean = 5, sd = 2) # set starting point for each subject
initialAffect2 <- rnorm(n = NSubjects, mean = 5, sd = 2) # set starting point for each partner
  ...
  for(obsi in 1:Nobs){ #for each observation of a subject
    ...
    if(obsi>1){ #else compute new affect state by taking a sequence of small steps in time
      for(stepi in 1:Nsteps){ #take Nsteps in time between each observation
        ...
        dAffect2 <- A*Affect2State + 0 * Affect1State + B
...
# create plot for 1 couple only
p2 = ggplot(data[data$Subject==1,], # Plot the data for the first couple
